...
| 情况 | `skip_paused` | `fill_paused` | 返回行为 |
| --- | --- | --- | --- |
| 整日停牌 | `False` | `True` | 保留当日，OHLC 延续前收盘、量额为 0、`paused=1` |
| 整日停牌 | `False` | `False` | 保留当日，行情和 `paused` 为 NaN |
| 整日停牌 | `True` | 任意 | 删除当日；`count` 向前取足可用行 |
| 盘中暂停且请求截止前已复牌 | 任意 | 任意 | 保留标准分钟轴，暂停段量额为 0、`paused=0` |

整日停牌的分钟查询保留 240 个交易分钟或按 `skip_paused=True` 删除。盘中暂停的缺分钟必须有 QMT 明确的暂停标记；当日开头的暂停段使用查询截止前可见的首根真实分钟开盘价，中间暂停段使用之前真实分钟收盘价。真实成交行保留 QMT 原始量价，再由既有流程统一复权。

若查询结束在当日复牌之前、尚无当日可见成交价，服务端明确报告价格依赖不足，不读取之后的复牌价格。公开 `get_price` 沿用现有异常处理，会记录错误并返回空表。这个边界与聚宽事后历史可能回填未来开盘价的行为有区别。

已验证样本：513100 在 2026-09-24 的盘中暂停，以及 000929 在 2026-06-15 的整日停牌。对齐的是时间轴和停牌语义，数据源之间的真实量价精度差异仍保留。此修复只需更新 server Python 代码，不要求更换 helper。
