...
正式版承接 0.10.0 Beta 系列的数据源与实盘能力，并整合下述 Tick/期货回测、行情兼容和回归测试。华鑫原生 C++ 库本次未重新验收，相关能力不因本次正式版声明扩大支持范围。
...
### 修复
- **大 QMT 盘中停牌分钟缺行**：区分整日与盘中暂停，依据明确分钟状态补齐已复牌交易日的分钟轴，修复查询报错；API 默认值和整日停牌处理保持不变。暂停段仅使用查询截止内可见的成交价格，复牌前缺少当日价格时明确失败，防止引用未来开盘价。增加 513100 盘中暂停和 000929 整日停牌的冻结市场样本回归；真实成交行量价不作跨源修整。
- **历史现金证券撮合与费用**：历史市价单预算不再应用实时保护价预留；货币基金继承显式基金费用，保留细分类别、标的覆盖和零费率。最低佣金不重复预留，全部清仓支持零股；现金预算及精度使用显式配置，见 [现金证券回测](docs/equity-backtest.md)。
- **历史估值与统计**：回调前按可见历史行情估值；严格数据模式的读取失败不能被策略捕获后掩盖。公司行为、双边费用与已实现盈亏贯通交易导出，报告统一风险统计口径及基准前一交易日基值。
- **日志与期货委托边界**：策略日志支持百分号、花括号及普通多参数，保留异常、时间和附加字段；关闭级别不提前格式化。旧枚举形式的期货市价单同样执行日 K 校验，严格数据异常继续向上传递。
- **tick 事件与行情一致性**：同刻多笔行情不再重复运行定时任务，收盘任务在同刻末笔行情之后执行；当前行情只读取已投递快照，回调前更新期货持仓权益，无快照时拒绝撮合。
- **期货平仓与保证金**：计费和库存扣减使用同一份今昨仓分配；存量持仓在日期费率生效后补退保证金，延迟目标单按合约和多空方向分别去重。
- **期货成交量约束与精度**：日线/分钟期货市价单检查当日日 K 的有效性与滑点前基准价域，每笔独立以完整日成交量为上限；费用计算保留原精度，期货指数行情不按实物合约档位取整。
- **期货成交统计**：新成交保存方向、开平和实际乘数，按多空分别归集并分摊开仓费用；导出保留这些字段。旧数据无法确定空头或乘数时明确报错，避免输出错误的胜率与盈亏比。
...
- **期货交易统计漏乘合约乘数**：报告层按「价差 × 手数 × 成交乘数」计算期货回合盈亏，股票口径不变。旧成交缺少乘数时仅对可识别的纯多头记录解析合约规格，解析失败明确报错。
...
### 文档
- **跨源停牌验收**：保留 000929 整日停牌和 513100 盘中暂停的固定日期、证券类型、参数矩阵及聚宽响应；19 组请求支持通过现有 `--live-providers` 对不同数据源重复验收。
