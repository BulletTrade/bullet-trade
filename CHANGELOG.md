...
### 新增
- **tick 频率回测**：回测引擎支持 `frequency='tick'`，调度时点与历史逐笔事件合并进同一条时间轴，按数据源原始时间戳排序后依次投递；`handle_tick` 逐笔触发，`get_current_tick` 读取回放缓冲而非实时行情。订阅门控与实盘一致：盘前订阅当日生效，盘中新增订阅不回填更早的 tick，`unsubscribe_all()` 停止投递。
- **按日 tick 预取与缓存**：按自然日整段拉取已订阅合约并落本地 parquet 缓存，tick 循环内不再同步访问上游；已订阅合约缺当日数据时抛 `TickDataMissingError`（信息含合约与日期），不降级为分钟/日线代理，也不回落实时行情。
- **期货回放账本**：回测下期货持仓记入独立账本，日终按结算价盯市并把变动结转进现金、保证金结转到结算价基准、到期合约了结并释放保证金；缺结算价只告警并保持上一基准，不做替代估值。合约乘数、最小变动价位与保证金率按上文「期货合约规格分层来源」解析，手续费率需显式配置，缺失时显式失败。
- **策略接口补齐**：`run_daily` 补上 `reference_security` 与 `force` 参数，与 `run_weekly` / `run_monthly` 口径一致；订单对象补齐成交相关字段（`status`、`add_time`、`is_buy`、`amount`、`filled`、`price`、`avg_cost`、`side`、`action`、`commission`）。
- **期货合约规格分层来源**：合约乘数、最小变动价位与保证金率不再只依赖代码内置规格表，改为按「显式覆盖 > 显式登记 > 外部配置 > 远端数据源 > 内置兜底」的顺序解析。外部配置为 `bullet_trade/config/futures_contract_specs.json`，支持合约级覆盖、按品种的保证金率生效日期分段，以及远端不可达时的离线兜底规格；数据源接口可替换，便于接入自有数据库或文件。规格按**合约**而非品种解析，同品种不同月份合约的最小变动价位可以不同。主力、连续、指数等不可撮合的伪合约在规格查询路径上显式拒绝，只能用于行情信号。
- **期货目标单**：`order_target` / `order_target_value` 补齐 `side`、`pindex`、`close_today` 参数，与 `order()` 的方向语义一致；期货的目标价值按占用保证金预算解释，手数按 `价值 / 价格 / 保证金率 / 乘数` 的顺序截断反解，当前持仓按方向从期货账本读取。

### 修复
- **tick 回测下的市价单保护价**：撮合基准在 tick 模式已取当前快照价，市价单保护价却仍按 bar 行情计算，两个基准的价差一旦超过保护带就会把订单误判为越界取消，策略侧只看到"未成交"。改为有 tick 快照时保护价与撮合基准同源。
- **回测订单时间戳**：`Order.add_time` 此前写入的是运行回测时的墙钟时间，现改为回放时刻，且只在订单首次登记时写入，避免跨时刻未成交挂单的下单时间被后移。
- **整日预取被未来数据守卫拦下**：`avoid_future_data=True` 时引擎自身的按日预取会被面向策略的守卫拒绝，导致 tick 回测无法启动。新增仅在引擎内部预取期间生效的守卫暂停作用域，策略侧查询的未来数据限制不变。
- **期货目标单方向与当前持仓**：目标单此前固定按买入方向派发，平多会被当成开多；当前持仓又只从股票持仓视图读取，期货持仓恒为 0，于是每次调仓都在已有仓位上继续叠加。改为按订单方向派发，并从期货账本按方向读取当前持仓。
- **期货目标单首笔调仓整批被拒**：保证金率是在建立期货账本时才从 `set_option` 同步进合约规格表的，而该调用此前只发生在撮合阶段，晚于下单尺寸计算；期货目标单要先按保证金预算反解手数，于是策略的第一笔调仓全部以「缺少保证金率」被拒，回测全程零成交。按显式手数下单不受影响。改为在尺寸计算之前对期货订单先确保账本已建立。
- **撮合价格有效性漏掉 NaN**：撮合基准价把 `NaN` 当作有效价返回，盘口价的有效性判据用 `price <= 0`，而 `NaN <= 0` 为假，两处都会让 `NaN` 进入成交价并污染持仓估值。改为统一用 `not price > 0` 判定。
- **最小变动价位取整位数硬编码**：撮合价按 tick 步长归整时保留位数写死为 2 或 3 位，步长更细的合约（如 0.005）会被抹平。改为由 tick 步长自身的小数位推导。
- **期货交易统计漏乘合约乘数**：报告层的交易胜率与盈亏比按「价差 × 手数」计盈亏，未乘合约乘数，而期货账本盯市一直按「价差 × 手数 × 乘数」计算。漏乘让手续费相对毛利被放大乘数倍，价格小幅波动的期货回合会被误判为亏损，胜率偏低、盈亏比失真。改为按标的解析合约乘数；股票口径不变，规格解析失败时退化为乘数 1 而不是中断报告生成。
- **期货成交价被 A 股最小价差二次归档**：撮合价先按合约规格表的最小变动价位归档，随后的价格笼子裁剪又用 A 股默认档位 `0.01` 重新归档一次。最小变动价位细于 `0.01` 的合约（如国债期货 `0.005`）会被压到 `0.01` 网格上，且内置 `round()` 的银行家舍入让结果在相邻两档之间不规则跳变。改为 `pricing.get_min_price_step` 对期货返回 `0`（档位未知，调用方跳过归档），期货档位只由合约规格表解析；涨跌停裁剪行为不变。
- **档位取整被浮点尾差挤掉一档**：按档位取整时用「价格 / 步长」得到档位序号再取整，而该除法在二进制下带尾差——`451.78 / 0.02` 是 `22588.999999999996`，`226.45 / 0.05` 是 `4528.999999999999`。已经落在档位上的合法报价会被向下取整挤掉整整一档，变成非法价格。原先各分支自带的 `1e-12` 绝对容差在档位序号达到几万量级时不够用（尾差约 `1e-11`）。改为在取整前按 9 位小数统一归零档位序号：真实的档位内偏移不会小于 `1e-4`，与该量级的尾差之间有五个数量级的间隔。
- **期货加仓后新增手数的盯市起点错误**：持仓的盯市基准只在首次开仓时写入，此后加仓不更新，而日终结算按「结算价 − 盯市基准」对**整个**持仓计算盯市变动，于是新增手数被从上一结算价而不是从自己的成交价起算，当日结算把 `(成交价 − 上一结算价) × 手数 × 乘数` 当成浮动盈亏计进权益与现金；同一缺陷也让盘中持仓估值偏高。改为加仓时按手数加权合并盯市基准，即 `新基准 = (原手数 × 原基准 + 加仓手数 × 成交价) / 合并后手数`。保证金侧不受影响：新增保证金仍按成交价冻结，结算时的保证金追加额与合并前逐分一致，变化的只有盯市变动与当日盈亏。首次开仓（原手数为 0）退化为「基准 = 成交价」，与原行为相同。

### 文档
- **tick 回测指南**：`docs/tick.md` 增补 tick 频率回测的事件模型、订阅门控、数据获取与失败行为、回放合成行情的 `tick_replay` 代理标识、订单时间戳口径、期货结算与费用配置要求，以及 CLI 与异步引擎尚不支持 tick 的限制说明。
- **期货合约规格与保证金率**：`docs/backtest.md` 新增一节，说明规格的分层解析顺序与「按合约而非按品种」的原因、外部 JSON 配置的形状与可替换来源接口、保证金率四级优先级、期货目标价值按保证金预算反解手数的口径，以及主力/连续/指数伪合约不可撮合、规格缺失即拒单这两条限制。

### 兼容性与升级
- **期货保证金率优先级反转（行为变更）**：此前引擎只读全局 `set_option('futures_margin_rate', x)`，且全局值优先于按品种设置，因此形如 `set_option('futures_margin_rate.T', 0.03)` 的按品种覆盖会被全局值压掉，实际仍按全局保证金率计费。现按「按品种覆盖 > 全局覆盖 > 外部配置（含生效日期分段）> 合约规格自带保证金率」的顺序取值。**同时设置了全局与按品种保证金率的策略，资金占用与可开手数会发生变化，升级后需要复核回测结果。**
- **期货合约规格缺失改为显式失败**：无法解析出合约乘数或保证金率时不再退回隐含默认值，订单被拒并记录 `futures_contract_spec_missing`；主力、连续、指数等伪合约进入下单或规格查询路径时直接报错，只允许用于行情信号。依赖旧行为静默成交的策略需要显式登记规格或补充外部配置。
- **期货撮合价改为按档位向下截断（行为变更）**：滑点后的价格落在两个档位之间时，此前四舍五入到最近档位，现改为向下截断到所在档位，成交价不再越过滑点后价格；显式给出方向的委托仍按方向取整，股票、ETF 等非期货标的仍按四舍五入。**所有配置了滑点的期货回测成交价会变化（最多一个最小变动价位），并通过保证金预算影响反解出的手数，升级后需要复核回测结果。**
- **期货加仓盯市基准修正（行为变更）**：新增手数的盯市起点由上一结算价改为成交价。**凡是在已有隔夜持仓上继续加仓的期货回测，当日结算的盯市变动、权益曲线与现金都会变化；每次加仓的偏差量为 `(成交价 − 上一结算价) × 手数 × 乘数`，方向可正可负，升级后需要复核回测结果。** 每个交易日都从空仓开始、当日了结、不在隔夜持仓上加仓的策略不受影响。保证金占用与手数反解口径不变。

### 测试
- 增加 tick 回放事件循环、订阅门控、缺数据显式失败、未来数据守卫边界、市价单保护价基准与订单时间戳的回归用例；期货账本与撮合用例覆盖多空开平、今昨仓拆分与平今计费、保证金不足与超平拒单、结算盯市与到期交割。全部使用合成数据，不依赖外部数据源。
- 增加合约规格分层来源用例：伪合约拒绝、合约级覆盖优先于品种级、显式登记优先于外部来源、四级保证金率优先级、按品种生效日期分段、非法配置条目丢弃、配置文件缺失时退化为空配置、远端批量拉取与缓存、拉取异常不外泄、链式来源首个命中即返回。增加期货目标单用例：目标手数按方向读账本、目标价值按保证金预算反解并截断、按品种保证金率参与手数计算、NaN 价格不被当作有效撮合价、细价位合约取整不被抹平。全部使用合成数据。
- 增加期货交易统计用例：合约乘数只对期货标的生效、期货回合的胜负判定依赖乘数（漏乘会把小幅盈利误判为亏损）、盈亏比随乘数缩放、股票交易统计口径不变。全部使用合成数据。
- 增加期货档位归属用例：`pricing.get_min_price_step` 对期货返回 `0` 表示档位未知、价格笼子裁剪不把 `0.005` 档位上的成交价压到 `0.01` 网格、期货在未指定方向时按档位向下截断而股票仍四舍五入、带浮点尾差的在档价格不被挤掉一档且方向取整仍按方向生效。全部使用合成数据。
- 增加期货加仓盯市基准用例：隔夜结算后次日在更高价位加仓，断言盯市基准按手数加权合并、新增保证金仍按成交价冻结、当日结算只计入原持仓的盯市变动而新增手数不再贡献虚假浮动盈亏。使用合成数据。
