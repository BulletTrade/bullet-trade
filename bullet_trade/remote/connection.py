...
文件职责: 管理通用远程券商协议的客户端长连接与请求副作用语义。
主要输入: 远程 action、请求载荷和连接配置。
主要输出: RPC 响应、事件回调及写入结果不确定异常。
上游关系: RemoteQmtBroker、RemoteQmtProvider 和生产控制任务。
下游关系: bullet-trade server 握手、心跳、请求与事件协议。
关键配置: 模糊交易写入绝不自动重发；幂等转换和只读请求最多重试一次。
"""
...
class RemoteQmtConnection:
    ...
    def resolve_submission(
        ...
        *,
        ...
        Returns:
            Dict[str, Any]: 服务端返回的解析结果。

        Raises:
            ValueError: 原 action 不是受支持的模糊写请求时抛出。
        """
    ...
    async def _request_async(self, action: str, payload: Dict) -> Dict:
        ...
        Raises:
            RemoteSubmissionUnknownError: 模糊写可能已送达但响应无法确认时抛出。
            Exception: 服务端明确错误或有界重试失败时原样抛出。
        """
